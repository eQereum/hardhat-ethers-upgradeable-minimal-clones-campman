...
describe('FactoryV2 Contract -----------------------------------------------------', () => {
  ...
  let campaignCloneTargetAddress, tokenCloneTargetDeployAddress, poolCloneTargetDeployAddress;
  ...
  before(async () => {
    ...
    // for upgrading we just override hardhatCampaignFactory after upgrade;
    const newImplementation = await ethers.getContractFactory('FactoryV2');
    hardhatCampaignFactory = await upgrades.upgradeProxy(campaignFactoryAddress, newImplementation);
  });
